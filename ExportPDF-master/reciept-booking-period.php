...
if (isset($_GET["b_id"])) {
    $id = $_GET["b_id"];
    $resultdata = mysqli_query($conn, "SELECT * FROM market_detail,booking_period,stall,provinces,amphures,districts,opening_period WHERE booking_period.stall_id=stall.sKey and stall.market_id = market_detail.mkr_id AND market_detail.province_id = provinces.id and market_detail.	amphure_id = amphures.id and market_detail.district_id = districts.id and booking_period.op_id = opening_period.id and b_id= '$id'");
